...
#' @param list_of_times a numeric vectors indicating the dates in simulation. Note that date format is not used.
...
incidence_vector <- function(max_age, list_of_times,
                             incidence, time_step)
{
...
incidence_matrix <- function(max_age, list_of_birth_times,
                             incidence, time_step)
{
  # populates an incidence matrix function based on the incidence function supplied
  # the maximum age, list of times and the required time-step. Note the approximation
  ...
  incidence_matrix <-  matrix(NA, nrow = length(list_of_birth_times) , ncol =  length(ages))
...
# incidence_matrix_surveydates <- function(max_age = 5, list_of_birth_times,
#                                          incidence, time_step,
#                                          survey_dates)
# {
#   # populates an incidence matrix function based on the incidence function supplied
...
#   # of being infected in the interval in question is calculated at mid point.
#
#   survey_rows <- match(survey_dates ,list_of_birth_times)
#
#   ages  <- seq(time_step, max_age, time_step)
...
#   }
#
#   incidence_matrix_survey <- incidence_matrix[survey_rows, ]
#
#   return(incidence_matrix_survey)
# }
#
