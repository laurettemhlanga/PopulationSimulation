...
infected_mortality_array <- function(age_steps,
                                     birth_dates,
                                     delta = 1,
                                     ...
                                     generate_base_mortality_fun)
{
  ...
  for (aa in ages){
    for (ta in ages[-1]){
      ...
      infected_mortality_array[times + aa + 1, aa + 1, ta ] = exp(-(generate_base_mortality_fun(times + aa , aa ) * generate_excess_mortality_tau_fun(times + aa, aa, ta)) * delta)
...
infected_survival_probs <- infected_mortality_array (age_steps = 3, birth_dates = 1945:1950,
                                                     generate_excess_mortality_tau_fun = generate_excess_mortality_tau,
                                                     generate_base_mortality_fun = generate_base_mortality)
dim(infected_Survival_probs)

exp(-(generate_base_mortality_fun(2 , 1 ) * generate_excess_mortality_tau_fun(2, 1, 1)) * 1)
...
infected_mortality_array_1 <- function(age_steps, birth_dates,
                                                delta = 1,
                                                generate_excess_mortality_tau_fun,
                                                generate_base_mortality_fun)
{
  ...
  for (aa in ages){
    for (ta in ages){
      ...
      infected_mortality_array[times + aa, aa, ta ] = exp(-(generate_base_mortality_fun(times + aa , aa - 1) * generate_excess_mortality_tau_fun(times + aa, aa, ta)) * delta)
      # R counts from 1 and hence 1 is added to account for the age 0
    }
  }
